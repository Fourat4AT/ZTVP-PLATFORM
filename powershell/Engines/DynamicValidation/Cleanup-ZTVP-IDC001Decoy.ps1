...
if (-not (Test-Path $statePath)) {
    ...
    Write-Host ""
    Write-Host "No active ID-C-001 decoy state was found."
...
$assignmentId = [string]$state.role_assignment.assignment_id

$roleCleanup = [PSCustomObject]@{
    attempted = $false
    removed = $false
    action = "None"
    error = $null
...
if (-not [string]::IsNullOrWhiteSpace($assignmentId)) {
    $roleCleanup.attempted = $true

    try {
        Invoke-MgGraphRequest -Method DELETE -Uri "https://graph.microsoft.com/v1.0/roleManagement/directory/roleAssignments/$assignmentId" | Out-Null
        $roleCleanup.removed = $true
        $roleCleanup.action = "Removed"
    }
    catch {
        ...
        if (Test-ZTVPNotFound -Message $message) {
            $roleCleanup.removed = $true
            $roleCleanup.action = "AlreadyAbsent"
            $roleCleanup.error = $null
        }
        else {
            $roleCleanup.removed = $false
            $roleCleanup.action = "RemoveFailed"
            $roleCleanup.error = $message
        }
    }
}
...
try {
    $state | ConvertTo-Json -Depth 30 | Set-Content -Path $archivePath -Encoding UTF8
}
catch {}
...
Write-Host ""
Write-Host "ID-C-001 fresh decoy cleanup completed"
Write-Host "Exact user: $userUpn"
...
Write-Host "Archived state: $archivePath"
Write-Host ""
