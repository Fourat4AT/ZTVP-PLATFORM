...
$cleanupPath = Join-Path $scenarioDir "idv006-cleanup-result.json"
...
if (-not (Test-Path $statePath)) {
    $result = [PSCustomObject]@{
        ...
        message = "No active ID-DV-006 state file was found."
        cleaned_at = (Get-Date).ToString("s")
    }
    Write-ZTVPJson -Path $cleanupPath -Object $result
    Write-Host "No active ID-DV-006 state file was found."
    exit 0
}
...
if (-not [string]::IsNullOrWhiteSpace($decoyUserId)) {
    try {
        ...
        $actions += "Deleted ID-DV-006 decoy user."
    }
    catch {
        $msg = $_.Exception.Message
        if (Test-ZTVPNotFound -Message $msg) {
            $actions += "ID-DV-006 decoy user was already removed."
        }
        else {
            $errors += "Could not delete ID-DV-006 decoy user: $msg"
        }
    }
}
...
$archivePath = Join-Path $historyDir "idv006-state-$runId.json"
...
Write-Host ""
Write-Host "ID-DV-006 cleanup completed."
