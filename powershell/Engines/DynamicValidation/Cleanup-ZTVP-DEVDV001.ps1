...
$cleanupPath = Join-Path $scenarioDir "devdv001-cleanup-result.json"
...
if (-not (Test-Path $statePath)) {
    $result = [PSCustomObject]@{
        ...
        message = "No active DEV-DV-001 state file was found."
        cleaned_at = (Get-Date).ToString("s")
    }

    Write-ZTVPJson -Path $cleanupPath -Object $result
    Write-Host "No active DEV-DV-001 state file was found."
    exit 0
}
...
if (-not [string]::IsNullOrWhiteSpace($decoyUserId)) {
    try {
        ...
        $actions += "Deleted DEV-DV-001 decoy user."
    }
    catch {
        $msg = $_.Exception.Message

        if (Test-ZTVPNotFound -Message $msg) {
            $actions += "DEV-DV-001 decoy user was already removed."
        }
        else {
            $errors += "Could not delete DEV-DV-001 decoy user: $msg"
        }
    }
}
...
$archivePath = Join-Path $historyDir "devdv001-state-$runId.json"
...
Write-Host ""
Write-Host "DEV-DV-001 cleanup completed."
